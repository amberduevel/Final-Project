...
jail_cc = function(space) {
  mono_board[space, 2] = as.numeric(mono_board[space, 2] + 1)
  cat("jail_cc", "before space update", player_df[player+1, 3], "\n")
  player_df[player+1, 2] = space
  cat("jail_cc", "after space update", player_df[player+1, 3], "\n")
  mono_graph_data = rbind(mono_graph_data, c(player_df[player+1, 1], mono_board[player_df[player+1, 2], 3], mono_board[player_df[player+1, 2], 4]))
  assign("player_df",player_df,.GlobalEnv)
  ...
  print(ls())
  cc_money(r_pos, player_df)
  ls()
  cat("jail_cc", "before assign", player_df[player+1, 3], "\n")
  assign("player_df",player_df,.GlobalEnv)
  assign("mono_board", mono_board, .GlobalEnv)
  assign("mono_graph_data", mono_graph_data, .GlobalEnv)
  cat("jail_cc", "after assign", player_df[player+1, 3], "\n")
  return(space) 
}
...
jail_c = function(space) {
  ...
  chance_money(r_pos)
  cat("jail_c", "before assign", player_df[player+1, 3], "\n")
  assign("player_df",player_df,.GlobalEnv)
  assign("mono_board", mono_board, .GlobalEnv)
  assign("mono_graph_data", mono_graph_data, .GlobalEnv)
  cat("jail_c", "after assign", player_df[player+1, 3], "\n")
  return(space)
}
```
...
cc_money<-function(r_pos, player_df){
  player_df[player+1,3] = player_df[player+1,3] + cc_cards[r_pos, 2]
  if(cc_cards[r_pos, 2] < 0){
    bank_money = bank_money - cc_cards[r_pos, 2]
    cat("cc_money", "before assign", player_df[player+1, 3], "\n")
    assign("player_df", player_df[,3], .GlobalEnv)
    cat("cc_money", "after assign", player_df[player+1, 3], "\n")
    #return(player_df)
  }
  else {
    cat("cc_money", "before assign", player_df[player+1, 3], "\n")
...
chance_money<-function(r_pos){
  ...
  if(chance_cards[r_pos, 2] < 0){
    ...
    cat("chance_money", "before assign", player_df[player+1, 3], "\n")
    assign("player_df", player_df, .GlobalEnv)
    cat("chance_money", "after assign", player_df[player+1, 3], "\n")
    #return(player_df)
  }
  else {
    cat("chance_money", "before assign", player_df[player+1, 3], "\n")
...
move = function() {
  ...
  cat("move", "before assign", player_df[player+1, 3], "\n")
  assign("mono_board", mono_board, .GlobalEnv)
  assign("player_df", player_df, .GlobalEnv)
  assign("mono_graph_data", mono_graph_data, .GlobalEnv)
  cat("move", "after assign", player_df[player+1, 3], "\n")
  return(space)
}
